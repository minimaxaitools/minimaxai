...
class TouchGestureManager {
    ...
    handlePinchGesture(touch1, touch2) {
        ...
        if (this.lastTouchDistance > 0) {
            ...
            const zoomSpeed = 0.5; // Adjust for touch sensitivity

            // Apply zoom similar to mouse wheel
            let f = parseFloat(data.settings.zoomFactor);
            let oldRange = data.cam.range;

            // Convert zoom factor to wheel-like delta
            const wheelDelta = zoomFactor > 1 ? -10 : 10;
            const adjustedDelta = wheelDelta * zoomSpeed;

            let newRange = data.cam.targetRange.mul(Math.pow(f, adjustedDelta / 100));
            ...
            let offset = centerWorld.sub(data.cam.pos).mul(new Decimal(1).sub(scale));
        ...
        if (Math.abs(centerDeltaX) > 2 || Math.abs(centerDeltaY) > 2) {
            this.handlePanning(centerDeltaX, centerDeltaY);
        }
    ...
    handleLongPress(touch) {
        ...
        // Handle long press actions based on current tool
        if (data.settings.tool === TOOL_SELECT) {
