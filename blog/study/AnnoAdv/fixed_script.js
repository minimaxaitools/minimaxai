...
function startApp() {
        ...
        // ─── EXPOSE TO GLOBAL STATE (For external plugins) ───
        window.AppStore = {
                getAnnotations: () => annotations,
                setAnnotations: (arr) => annotations = arr,
                getObjects: () => objects,
                setObjects: (arr) => objects = arr,
                getCanvasSettings: () => canvasSettings,
                setCanvasSettings: (obj) => canvasSettings = obj,
                getIdCounter: () => idCounter,
                setIdCounter: (num) => idCounter = num,
                getSelectedId: () => selectedId,
                setSelectedId: (id) => selectedId = id,
                getSelectedObjId: () => selectedObjId,
                setSelectedObjId: (id) => selectedObjId = id,
                showToast: (msg, dur) => { if (typeof showToast === 'function') showToast(msg, dur); },
                captureState: () => { if (typeof captureState === 'function') captureState(); },
                render: () => { if (typeof render === 'function') render(); },
                updateUI: () => { if (typeof updateUI === 'function') updateUI(); },
                applyCanvasSettings: () => { if (typeof applyCanvasSettings === 'function') applyCanvasSettings(); },
                svgEl: svgEl,
                d3: d3
        };
        ...
        function renderAnnotation(ann) {
                ...
                function syncAnn(annObj) {
                        ...
                        // Force redraw to lock inline styles (like Badge Font) wiped structurally during d3-annotation drag
                        render();
                        // Trigger physics repulsion engine
                        if (typeof runAutoRepel === 'function') runAutoRepel(ann.id);
                }
                ...
                // ─── APPLY COLORS / STYLES ───
                window.applyCustomStylesFromTick(group, ann);
        ...
        function updateUI() {
                ...
                if (ann) {
                        ...
                        if (document.getElementById('badgeFont')) document.getElementById('badgeFont').value = ann.badgeFont || 'Inter';
                        if (document.getElementById('badgeFontSize')) document.getElementById('badgeFontSize').value = ann.badgeFontSize || 12;
                }
        ...
        function addAnnotation(x, y) {
                ...
                const ann = {
                        ...
                        badgeText: 'A', badgeRadius: 14, badgeFont: 'Inter', badgeFontSize: 12,
                        zIndex: 10,
                        disableParts: [],
        ...
        function bindSidebarEvents() {
                ...
                if (wsEl) {
                        wsEl.addEventListener('change', function () {
                                const ann = annotations.find(a => a.id === selectedId); if (!ann) return;
                                ann.wrapSplitterEnabled = this.checked;
                                if (!this.checked && ann.label) {
                                        ann.label = ann.label.replace(/\n/g, ' ').replace(/(^|\s)[\u2022▸→◆✓★–\u276f\-\*]\s/g, '$1');
                                        const editLabel = document.getElementById('editLabel');
                                        if (editLabel) editLabel.value = ann.label;
                                }
                                render();
                        });
                }
                ...
                // Apply Global Curve
                const applyGlobalCurve = document.getElementById('applyGlobalCurve');
                if (applyGlobalCurve) {
                        applyGlobalCurve.addEventListener('click', () => {
                                const type = document.getElementById('curveType').value;
                                const pts = +(document.getElementById('curvePoints').value || 2);
                                captureState();
                                annotations.forEach(a => {
                                        if (multiSelectedIds.size > 0 && !multiSelectedIds.has(a.id)) return;
                                        a.curveType = type; a.curvePoints = pts; a.connectorPoints = null;
                                });
                                render();
                                showToast('Curve options applied');
                        });
                }

                // Badge radius and text
                const badgeRadiusEl = document.getElementById('badgeRadius');
                if (badgeRadiusEl) {
                        badgeRadiusEl.addEventListener('input', function () {
                                const ann = annotations.find(a => a.id === selectedId); if (!ann) return;
                                ann.badgeRadius = +this.value; render();
                        });
                }
                const badgeTextEl = document.getElementById('badgeText');
                if (badgeTextEl) {
                        badgeTextEl.addEventListener('input', function () {
                                const ann = annotations.find(a => a.id === selectedId); if (!ann) return;
                                ann.badgeText = this.value; render();
                        });
                }
                const badgeFontEl = document.getElementById('badgeFont');
                if (badgeFontEl) {
                        badgeFontEl.addEventListener('input', function () {
                                const ann = annotations.find(a => a.id === selectedId); if (!ann) return;
                                ann.badgeFont = this.value; render();
                        });
                }
                const badgeFontSizeEl = document.getElementById('badgeFontSize');
                if (badgeFontSizeEl) {
                        badgeFontSizeEl.addEventListener('input', function () {
                                const ann = annotations.find(a => a.id === selectedId); if (!ann) return;
                                ann.badgeFontSize = +this.value; render();
                        });
                }
                
                // Apply Global Badge
                const applyGlobalBadge = document.getElementById('applyGlobalBadge');
                if (applyGlobalBadge) {
                        applyGlobalBadge.addEventListener('click', () => {
                                const r = +(document.getElementById('badgeRadius').value || 14);
                                const f = document.getElementById('badgeFont').value;
                                const fs = +(document.getElementById('badgeFontSize').value || 12);
                                captureState();
                                annotations.forEach(a => {
                                        if (multiSelectedIds.size > 0 && !multiSelectedIds.has(a.id)) return;
                                        a.badgeRadius = r; a.badgeFont = f; a.badgeFontSize = fs;
                                });
                                render();
                                showToast('Badge options applied');
                        });
                }
                ...
                ['subjectRadius', 'subjectWidth', 'subjectHeight', 'annZIndex', 'badgeText', 'badgeRadius', 'badgeFont', 'badgeFontSize'].forEach(id => {
                        const el = document.getElementById(id);
                        if (!el) return;
                        el.addEventListener('input', function () {
                                const ann = annotations.find(a => a.id === selectedId); if (!ann) return;
                                if (id === 'annZIndex') ann.zIndex = +this.value;
                                else if (id === 'badgeText' || id === 'badgeFont') ann[id] = this.value;
                ...
                ['extendTop', 'extendBottom', 'extendLeft', 'extendRight'].forEach(id => {
                        const el = document.getElementById(id);
                        if (!el) return;
                        el.addEventListener('click', () => {
                                const amt = +(document.getElementById('extendAmount').value) || 0;
                                if (amt <= 0) return;
                                captureState();

                                let newW = canvasSettings.customWidth || 800;
                                let newH = canvasSettings.customHeight || 500;

                                if (id === 'extendRight') newW += amt;
                                if (id === 'extendBottom') newH += amt;

                                if (id === 'extendLeft') {
                                        newW += amt;
                                        annotations.forEach(a => { if (a.x != null) a.x += amt; if (a.nx != null) a.nx += amt; });
                                        objects.forEach(o => { if (o.x != null) o.x += amt; });
                                }
                                if (id === 'extendTop') {
                                        newH += amt;
                                        annotations.forEach(a => { if (a.y != null) a.y += amt; if (a.ny != null) a.ny += amt; });
                                        objects.forEach(o => { if (o.y != null) o.y += amt; });
                                }

                                canvasSettings.customWidth = newW;
                                canvasSettings.customHeight = newH;

                                if (canvasWidth) canvasWidth.value = newW;
                                if (canvasHeight) canvasHeight.value = newH;

                                applyCanvasSettings(); renderGrid(); render(); updateUI();
                                showToast('Canvas extended by ' + amt + 'px', 2000);
                        });
                });
        }
        ...
        async function getEmbeddedFontCSS() {
                let usedFamilies = new Set(['Inter']); // Inter as fallback
                svgEl.querySelectorAll('*').forEach(el => {
                        const fStyle = el.style.fontFamily;
                        const fAttr = el.getAttribute('font-family');
                        const val = fStyle || fAttr;
                        if (val) {
                                const cleanFont = val.replace(/['"]/g, '').split(',')[0].trim();
                                usedFamilies.add(cleanFont);
                        }
                });

                annotations.forEach(a => {
                        if (a.titleFont) usedFamilies.add(a.titleFont);
                        if (a.labelFont) usedFamilies.add(a.labelFont);
                });

                const usedFonts = Array.from(usedFamilies).filter(f => FONTS.includes(f));
                if (usedFonts.length === 0) return '';

                const fontUrl = 'https://fonts.googleapis.com/css2?'
                        + usedFonts.map(f => 'family=' + f.replace(/ /g, '+') + ':wght@400;600;700').join('&')
                        + '&display=swap';

                try {
                        showToast('Embedding fonts (this may take a moment)...', 4000);
                        ...
                        const base64Results = await Promise.all(uniqueUrls.map(url =>
                                fetch(url)
                                        .then(r => r.blob())
                                        .then(blob => new Promise(resolve => {
                                                const reader = new FileReader();
                                                reader.onload = () => resolve({ url, b64: reader.result });
                                                reader.readAsDataURL(blob);
                                        }))
                                        .catch(() => null)
                        ));

                        base64Results.forEach(item => {
                                if (item) {
                                        cssText = cssText.split(item.url).join(item.b64);
                                }
                        });
                        return cssText;
                } catch (e) {
        ...
        async function exportSVG() {
                ...
                // Embed fonts
                const cssText = await getEmbeddedFontCSS();
                ...
                const w = canvasSettings.customWidth || 800;
                const h = canvasSettings.customHeight || 500;
        ...
        // ─── DYNAMIC FORCE COLLISION ENGINE (Auto Repel) ───
        window.applyCustomStylesFromTick = function(group, ann) {
                ...
                group.selectAll('.connector-dot').style('fill', c).style('stroke', c);
                group.selectAll('path.subject-ring').style('fill', 'white').style('stroke', c).style('stroke-width', '3px');
                ...
                const badgeFs = ann.badgeFontSize || 12;

                group.selectAll('.badge-text')
                        .style('font-family', `'${ann.badgeFont || 'Inter'}', sans-serif`)
                        .style('font-size', badgeFs + 'px');
                ...
                if (titleNode && labelNode) {
                        try {
                                ...
                                const expectedLabelY = tBox.y + tBox.height + 4;
        ...
        let autoRepelSim = null;
        window.runAutoRepel = function(droppedNodeId) {
                const modeStr = document.getElementById('autoRepelMode')?.value || 'none';
                if (modeStr === 'none') {
                        if (autoRepelSim) { autoRepelSim.stop(); autoRepelSim = null; }
                        return;
                }

                let R = 130;
                if (modeStr === 'compact') R = 80;
                if (modeStr === 'loose') R = 230;

                if (!autoRepelSim) {
                        autoRepelSim = d3.forceSimulation(annotations)
                                .force("collide", d3.forceCollide().radius(d => {
                                        return (d.typeKey === 'annotationCalloutCircle') ? (d.subjectRadius || 50) + 10 : R;
                                }).iterations(2))
                                .force("charge", d3.forceManyBody().strength(-15))
                                .on("tick", () => {
                                        annotations.forEach(d => {
                                                const group = d3.select('#ann-' + d.id);
                                                if (!group.empty()) {
                                                        const make = annotation().type(TYPE_MAP[d.typeKey] || annotationLabel)
                                                                .annotations([buildConfig(d)])
                                                                .editMode(d.id === selectedId);
                                                        group.select('.annotation').remove();
                                                        try { group.call(make); } catch (e) { }
                                                        window.applyCustomStylesFromTick(group, d);
                                                }
                                        });
                                });
                }
                ...
                annotations.forEach(d => {
                        if (d.id === droppedNodeId) { d.fx = d.x; d.fy = d.y; }
                        else { d.fx = null; d.fy = null; }
                });

                autoRepelSim.nodes(annotations);
                autoRepelSim.alpha(0.6).restart();
        };
