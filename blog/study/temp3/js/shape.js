...
class ShapeText extends Shape {
    ...
    isRenderable() {
        // Use size for height and estimate width based on text length (approximate)
        // or just use a safe margin since exact culling isn't critical for text
        let margin = this.size * (this.text.length + 1);
        let d = new Decimal(this.size).div(data.cam.range);

        if (!d.gte(1e-3) || !d.lte(1e3)) return false;

        const screenPos = this.getScreenPos(this.pos);
        const screenSize = this.getScreenSize(new Decimal(margin));
